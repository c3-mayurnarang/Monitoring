...
ACCESS_KEY="AKIAJCLJMCLBMIRDPRLA"
SECRET_KEY="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"
...
#hostname stage-paas-mon-02.stage-01-usw2.internal
if vgs | grep -q 'dataMonitoring'; then
  ...
  curl -L 'https://github.com/prometheus/prometheus/releases/download/v2.6.0/prometheus-2.6.0.linux-amd64.tar.gz' -o prometheus-2.6.0.linux-amd64.tar.gz
  tar -zxvf prometheus-2.6.0.linux-amd64.tar.gz
  ln -s /opt/monitoring/prometheus/prometheus-2.6.0.linux-amd64 /opt/monitoring/prometheus/current
      ...
      if [[ -e $PIDFILE ]];then
        ...
        if [[ -e $APP_PATH$APP_NAME ]];then
          ...
          if [[ $PRPORTEST ]]; then
            ...
            /usr/bin/setsid $APP_PATH$APP_NAME --config.file=$CONF_FILE --storage.tsdb.path=/opt/monitoring/prometheus/data&
            sleep 5
...
scrape_configs:
  ...
  - job_name: '$ENV_NAME-$CUST_NAME'
    ec2_sd_configs:
      - region: $ENV_REGION
        ...
        port: 9100
    relabel_configs:
      - source_labels: [__meta_ec2_tag_host_name]
        regex: ($ENV_NAME-$CUST_NAME-app-m-.*c3-prod.internal)|($ENV_NAME-$CUST_NAME-cass-.*c3-prod.internal)|(prod-c3-tibco-.*c3-prod.internal)|(prod-c3-zk-.*c3-prod.internal)
        action: keep
  ...
  - job_name: 'prod-c3-zk'
    ...
    relabel_configs:
      - source_labels: [__meta_ec2_tag_host_name]
        regex: prod-c3-zk-.*c3-prod.internal
        action: keep
      - source_labels: [__meta_ec2_tag_Name]
        target_label: instance
      - replacement: $ENV_NAME
